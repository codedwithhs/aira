...
def test_load_config_success(temp_config_dir: Path):
    ...
    config_data = {
        ...
        'connections': {
            'my_github': {'type': 'github', 'token': 'gh-token', 'default_repo': 'org/repo'},
            'my_pagerduty': {'type': 'pagerduty', 'api_key': 'pd-key', 'from_email': 'test@test.com'}
        },
        'actions': {
            'my_slack': {'type': 'slack', 'webhook_url': 'http://test.com'}
        }
    }
...
def test_load_config_missing_required_llm_section(temp_config_dir: Path):
    ...
    config_data = {
        'connections': {'my_github': {'type': 'github', 'token': 'gh-token', 'default_repo': 'org/repo'}}
    }
    config_file = temp_config_dir / "config.yaml"
    with open(config_file, 'w') as f:
        yaml.dump(config_data, f)

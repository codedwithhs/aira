...
def test_connection_success(requests_mock, valid_github_config):
    ...
    requests_mock.get("https://api.github.com/user", json={"login": "testuser"}, status_code=200)
...
def test_fetch_recent_commits_success(requests_mock, valid_github_config):
    ...
    repo = "test/repo"
    mock_response = [
        {"sha": "a1b2c3d4e5f6", "commit": {"author": {"name": "Test User"}, "message": "feat: Add new feature"}},
        {"sha": "f6e5d4c3b2a1", "commit": {"author": {"name": "Test User"}, "message": "fix: Correct a bug\n\nMore details here."}}
    ]
    requests_mock.get(f"https://api.github.com/repos/{repo}/commits", json=mock_response, status_code=200)
...
def test_fetch_recent_commits_no_commits(requests_mock, valid_github_config):
    ...
    connector = GitHubConnector(name="test_github", config=valid_github_config)
    repo = "test/repo"
    requests_mock.get(f"https://api.github.com/repos/{repo}/commits", json=[], status_code=200)
    result = connector.fetch_recent_commits(repo=repo, hours=1)

...
def init(
    ...
    if config_file_path.exists() or env_file_path.exists():
        if not typer.confirm(f"⚠️ Config files may already exist in {output_dir}. Overwrite?"):
...
def doctor(
    ...
    retries: int = typer.Option(1, "--retries", help="Number of times to retry a failed connection test."),
    retry_delay: int = typer.Option(2, "--retry-delay", help="Seconds to wait between retries.")
):
    """
    ...
    try:
        if not config_path.is_file():
            console.print(f"❌ [bold red]Error:[/bold red] Configuration file not found at [yellow]{config_path}[/yellow]")
            raise typer.Exit(code=1)
        ...
        if not any(components_to_test.values()):
            console.print("[yellow]⚠️ No connectors or LLM providers configured.[/yellow]")
        ...
        for name, component in components_to_test.items():
            ...
            for attempt in range(retries + 1):
                with console.status(f"[bold green]Testing '{name}' (attempt {attempt + 1})...[/bold green]"):
                    try:
...
@app.command()
def analyze(
    trigger_file: Path = typer.Argument(..., help="Path to the trigger event YAML file."),
    config_path: Path = ConfigReadOption
):
    """
